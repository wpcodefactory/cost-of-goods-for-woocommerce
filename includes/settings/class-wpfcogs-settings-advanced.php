...
if ( ! class_exists( 'WPFCOGS_Settings_Advanced' ) ) :

class WPFCOGS_Settings_Advanced extends WPFCOGS_Settings_Section {
	...
	function __construct() {
		...
		add_action( 'wpfcogs_on_update', array( $this, 'fix_calculation_hooks_default_option' ) );
		parent::__construct();
	}
	 ...
	 *
	 * Necessary since version 3.4.1 for previous users, where all order calculation hooks were moved to `wpfcogs_new_order_hooks_for_cost_update` option, and some previous users had the option set without some important hooks.
	 *
	 * @version 3.4.3
	 * @since   3.4.3
	 *
	 * @todo    : Remove it sometime in the future, when all users have the 'wpfcogs_new_order_hooks_for_cost_update' option set properly.
	 *
	 * @return void
	 */
	...
	function get_settings() {
		...
		$general_opts = array(
			...
			array(
				...
				'desc_tip'      => __( 'Only triggers with meta not starting with <code>_alg_wc_cog_</code>.', 'cost-of-goods-for-woocommerce' ),
			...
			array(
				...
				'type'    => 'multiselect',
				'class'   => 'chosen_select',
				'default' => array_keys( wpfcogs()->core->orders->get_new_order_hooks_for_cost_updating() ),
				'options' => wpfcogs()->core->orders->get_new_order_hooks_for_cost_updating()
			),
			array(
...
return new WPFCOGS_Settings_Advanced();
