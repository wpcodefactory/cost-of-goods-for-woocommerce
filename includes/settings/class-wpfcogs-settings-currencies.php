...
if ( ! class_exists( 'WPFCOGS_Settings_Currencies' ) ) :

class WPFCOGS_Settings_Currencies extends WPFCOGS_Settings_Section {
	 ...
	 * @todo    [maybe] `wpfcogs_currencies_wmc`: add link to the plugin on wp.org?
	...
	function get_settings() {
		...
		$multicurrency_order_calculation_opts = array(
			...
			array(
				...
				'desc_tip'          => sprintf(__( 'The affected areas are the %s and the order page on the admin.', 'cost-of-goods-for-woocommerce' ), '<a href="' . wpfcogs_get_admin_orders_page_url() . '">' . __( 'order listing page', 'cost-of-goods-for-woocommerce' ) . '</a>'),
				'type'              => 'checkbox',
		...
		$wc_currency                          = wpfcogs()->core->get_default_shop_currency();
...
return new WPFCOGS_Settings_Currencies();
