...
if ( ! class_exists( 'WPFCOGS_Settings_Tools' ) ) :

class WPFCOGS_Settings_Tools extends WPFCOGS_Settings_Section {
	...
	function get_import_tool_cron_info() {
		...
		if ( empty( self::$run_import_tool_cron_output ) ) {
			$output = '';
			if (
				( ! $event_timestamp = wp_next_scheduled( 'wpfcogs_run_import_tool' ) ) &&
				isset( $_POST['wpfcogs_import_tool_cron'] ) // phpcs:ignore WordPress.Security.NonceVerification.Missing
			) {
				$output = '<br />';
			...
			array(
				...
				'desc'  => __( 'A tool created with the purpose of importing the cost meta from another plugin by replacing the cost meta.', 'cost-of-goods-for-woocommerce' ) . '<br /><br />' .
		           ...
		           sprintf(
			           ...
			           '<a href="' . admin_url( 'tools.php?page=wpfcogs-import-costs' ) . '">' . __( 'Tools > Import Costs', 'cost-of-goods-for-woocommerce' ) . '</a>'
		           ),
...
return new WPFCOGS_Settings_Tools();
