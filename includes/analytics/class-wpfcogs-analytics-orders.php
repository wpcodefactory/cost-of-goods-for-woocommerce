...
if ( ! class_exists( 'WPFCOGS_Analytics_Orders' ) ) :

	class WPFCOGS_Analytics_Orders {
		...
		function __construct() {
			...
			add_filter( 'wpfcogs_analytics_localization_info', array( $this, 'add_analytics_localization_info' ) );
			...
			add_filter( 'wpfcogs_analytics_orders_costs_total_validation', array( $this, 'add_costs_total_column_if_option_is_enabled' ) );
			...
			add_filter( 'wpfcogs_analytics_orders_profit_total_validation', array( $this, 'add_profit_total_column_if_option_is_enabled' ) );
		...
		function is_cost_and_profit_totals_enabled() {
			return (bool) apply_filters( 'wpfcogs_analytics_orders_cost_profit_totals_enabled', false );
		}
		...
		function is_individual_costs_enabled() {
			return (bool) apply_filters( 'wpfcogs_analytics_orders_individual_costs_enabled', false );
		}
		...
		function add_costs_select_orders_stats_total( $clauses ) {
			if ( apply_filters( 'wpfcogs_analytics_orders_costs_total_validation', false ) ) {
		...
		function add_costs_join_orders( $clauses ) {
			...
			//$clauses[] = "JOIN {$wpdb->options} wpo ON option_name LIKE '%wpfcogs_currencies_rates%'";
			return $clauses;
		}
		...
		function add_profit_select_orders_stats_total( $clauses ) {
			if ( apply_filters( 'wpfcogs_analytics_orders_profit_total_validation', false ) ) {
...
return new WPFCOGS_Analytics_Orders();
