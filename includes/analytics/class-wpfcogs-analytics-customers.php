...
if ( ! class_exists( 'WPFCOGS_Analytics_Customers' ) ) :

	class WPFCOGS_Analytics_Customers {
		...
		function add_analytics_localization_info( $info ) {
			$info['cost_and_profit_columns_enabled_on_customers'] = 'yes' ===wpfcogs_get_option( 'alg_wc_cog_analytics_customers_cost_and_profit_columns', 'no' );
			return $info;
		}
		...
		function add_costs_select_orders_stats_total( $clauses ) {
			if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_analytics_customers_cost_and_profit_columns', 'no' ) ) {
				$clauses[] = wpfcogs()->core->analytics->orders->get_order_costs_total_meta_select_clauses();
			}
			return $clauses;
		}
		...
		function add_costs_join_customers( $clauses ) {
			if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_analytics_customers_cost_and_profit_columns', 'no' ) ) {
				$clauses[] = wpfcogs()->core->analytics->orders->get_order_cost_meta_join_clauses();
			}
		...
		function add_profit_join_customers( $clauses ) {
			if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_analytics_customers_cost_and_profit_columns', 'no' ) ) {
				$clauses[] = wpfcogs()->core->analytics->orders->get_order_profit_meta_join_clauses();
			}
			return $clauses;
		}
		...
		function add_profit_select_orders_stats_total( $clauses ) {
			if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_analytics_customers_cost_and_profit_columns', 'no' ) ) {
				$clauses[] = wpfcogs()->core->analytics->orders->get_order_profit_total_meta_select_clauses();
			}
			return $clauses;
		}
...
return new WPFCOGS_Analytics_Customers();