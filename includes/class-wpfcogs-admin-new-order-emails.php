...
if ( ! class_exists( 'WPFCOGS_Admin_New_Order_Emails' ) ) {

	class WPFCOGS_Admin_New_Order_Emails {
		...
		function init() {
			...
			add_action( 'wpfcogs_admin_new_order_email_meta', array( $this, 'add_new_order_email_cog_title' ), 10, 2 );

			// Add individual item cost and profit.
			add_action( 'wpfcogs_admin_new_order_email_meta', array( $this, 'add_individual_item_cost_and_profit' ), 20, 2 );

			// Add total cost and profit.
			add_action( 'wpfcogs_admin_new_order_email_meta', array( $this, 'add_total_cost_and_profit' ), 20, 2 );

			// Add margin at the end.
			add_action( 'wpfcogs_admin_new_order_email_meta', array( $this, 'add_margin_at_the_end' ), 50, 2 );

			// Enable cog admin new order email meta.
			add_filter( 'wpfcogs_admin_new_order_email_meta_enabled', array( $this, 'enable_cog_admin_new_order_email_meta' ) );
		}
		...
		function is_total_cost_and_profit_email_enabled() {
			return (bool) apply_filters( 'wpfcogs_order_admin_new_order_email_total_enabled', false );
		}
		...
		function is_item_cost_and_profit_email_enabled() {
			return (bool) apply_filters( 'wpfcogs_order_admin_new_order_email_item_enabled', false );
		}
		...
		function create_cog_admin_new_order_email_meta( $order_obj, $sent_to_admin ) {
			if (
				...
				apply_filters( 'wpfcogs_admin_new_order_email_meta_enabled', false, $order_obj, $sent_to_admin )
			) {
				do_action( 'wpfcogs_admin_new_order_email_meta', $order_obj, $sent_to_admin );
			}
		}
				...
				foreach ( $items as $item_id => $item ) :
                    ...
                    <tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
                        ...
                        <td class="td font-family" style="vertical-align:middle;">
							<?php echo wp_kses_post( '<span style="color:red;">' . wpfcogs_format_cost( wc_get_order_item_meta( $item_id, '_alg_wc_cog_item_cost' ) ) . '</span>' ); ?>
                        </td>
                        <td class="td font-family" style="vertical-align:middle;">
							<?php echo wp_kses_post( '<span style="color:green;">' . wpfcogs_format_cost( wc_get_order_item_meta( $item_id, '_alg_wc_cog_item_profit' ) ) . '</span>' ); ?>
                        </td>
                    </tr>
		...
		function add_total_cost_and_profit( $order, $sent_to_admin ) {
			...
			$profit_placeholders = array(
				'%profit%'         => wpfcogs()->core->orders->format_order_column_value( $profit, 'profit' ),
			...
			$table_data          = array(
				array( __( 'Cost', 'cost-of-goods-for-woocommerce' ), ( '' !== $cost ? '<span style="color:red;">' . wpfcogs_format_cost( $cost ) . '</span>' : '' ) ),
				array( __( 'Profit', 'cost-of-goods-for-woocommerce' ), ( '' !== $profit ? '<span style="color:green;">' . $profit_html . '</span>' : '' ) ),
			);
			?>

			<?php echo wp_kses_post( wpfcogs_get_table_html( $table_data, $table_args ) ); ?>
			<?php
		}
