...
if ( ! class_exists( 'WPFCOGS_Bulk_Edit_Attr_Filtering' ) ) {


	class WPFCOGS_Bulk_Edit_Attr_Filtering {
		 ...
		 */
		function init() {
			add_filter( 'wpfcogs_bulk_edit_get_products_args', array( $this, 'get_products_args' ) );
			add_filter( 'wpfcogs_bulk_edit_get_child_products_args', array( $this, 'get_child_products_args' ) );
		}
		...
		function get_child_products_args( $args ) {
			...
			$filtered_attr = filter_input( INPUT_POST, 'wpfcogs_bulk_edit_filtered_attr', FILTER_UNSAFE_RAW, FILTER_REQUIRE_ARRAY );
				...
				<tr>
					...
					<td>
						<select
							...
							name="wpfcogs_bulk_edit_filtered_attr[<?php echo esc_attr( $taxonomy ); ?>][]"
		...
		function get_selected_taxonomies() {
			return wpfcogs_get_option( 'alg_wc_cog_bulk_edit_filterable_taxes', array() );
		}
	}
