...
if ( ! class_exists( 'WPFCOGS_WP_List_Bulk_Edit_Tool' ) ) :

	class WPFCOGS_WP_List_Bulk_Edit_Tool extends \WP_List_Table {
		...
		public function prepare_items() {
			...
			$per_page = $this->get_items_per_page( 'wpfcogs_bulk_edit_per_page', 20 );
			$args     = array(
				'paginate'       => true,
				'tax_query'     => array(),
				'posts_per_page' => $per_page,
				'paged'          => isset( $_GET['paged'] ) ? filter_var( wp_unslash( $_GET['paged'] ), FILTER_SANITIZE_NUMBER_INT ) : 1, // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
				'orderby'        => 'ID',
				'order'          => isset( $_GET['order'] ) ? strtoupper( sanitize_text_field( wp_unslash( $_GET['order'] ) ) ) : 'ASC', // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
				'type'           => ( ! empty( $types ) ? $types : array_merge( array_keys( wc_get_product_types() ), array( 'variation' ) ) ),
			);
			// Search.
			if ( isset( $_REQUEST['s'] ) && ! empty( $search_query = $_REQUEST['s'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
				if ( 'title' === get_option( 'alg_wc_cog_bulk_edit_tool_search_method', 'title' ) ) {
					$args['s'] = sanitize_text_field( wp_unslash( $search_query ) );
				} else {
					$data_store        = WC_Data_Store::load( 'product' );
					$ids               = $data_store->search_products( sanitize_text_field( wp_unslash( $search_query ) ), '', true, true );
					$post_in           = array_merge( $ids, array( 0 ) );
					$args['include'] = $post_in;
				}
			}
			// Tax query - Product tag.
			if ( isset( $_GET['product_tag'] ) && ! empty( $product_tag = sanitize_text_field( wp_unslash( $_GET['product_tag'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
		...
		function fix_paged_query_string_on_search_change() {
			...
			if (
				...
				'tools_page_bulk-edit-costs' !== $current_screen->id
			) {
				return;
			}
			if ( ! session_id() ) {
			...
			if (
				...
				(
					( ! isset( $_SESSION['wpfcogs_bulk_edit_cost_search'] ) || empty( $referer_search = $_SESSION['wpfcogs_bulk_edit_cost_search'] ) ) ||
					$referer_search != $current_search
				)
			) {
				if ( ! isset( $_COOKIE['wpfcogs_bulk_edit_cost_search'] ) || empty( $referer_search = $_COOKIE['wpfcogs_bulk_edit_cost_search'] ) || 
				$referer_search != $current_search ) {
			...
			$_SESSION['wpfcogs_bulk_edit_cost_search'] = isset( $_GET['s'] ) ? sanitize_text_field( wp_unslash( $_GET['s'] ) ) : '';

			if ( ! isset( $_COOKIE['wpfcogs_bulk_edit_cost_search'] ) ) {
				if ( isset( $_GET['s'] ) ) {
					$s = sanitize_text_field( wp_unslash( $_GET['s'] ) );
					setcookie( 'wpfcogs_bulk_edit_cost_search', $s, time() + 86400 ); 
				}
			} else {
				unset( $_COOKIE['wpfcogs_bulk_edit_cost_search'] );
				if ( isset( $_GET['s'] ) ) {
					$s = sanitize_text_field( wp_unslash( $_GET['s'] ) );
					setcookie( 'wpfcogs_bulk_edit_cost_search', $s, time() + 86400 ); 
				}
		...
		function get_tags_edit_field( $post_id ) {
			ob_start();
			echo '<select data-return_id="id" class="wc-taxonomy-term-search" multiple="multiple" style="width: 50%;" id="wpfcogs_bulk_edit_tool_product_tag" name="wpfcogs_bulk_edit_tool_product_tag[' . esc_attr( $post_id ) . '][]" data-placeholder="' . esc_attr( __( 'Search for a tag&hellip;', 'cost-of-goods-for-woocommerce' ) ) . '" data-taxonomy="product_tag" data-action="json_search_tags">';
		...
		public function column_default( $item, $column_name ) {
			...
			switch ( $column_name ) {
				...
				case '_alg_wc_cog_cost':
					...
					$value       = wpfcogs()->core->products->get_product_cost( $item->get_id() );
					if ( $show_profit ) {
						/* translators: %s: Profit value or "N/A". */
						$profit_html = sprintf( __( 'Profit: %s', 'cost-of-goods-for-woocommerce' ), ( '' != ( $profit = wpfcogs()->core->products->get_product_profit_html( $item->get_id(), wpfcogs()->core->products->product_profit_html_template ) ) ? $profit : esc_html__( 'N/A', 'cost-of-goods-for-woocommerce' ) ) );
					} else {
						$profit_html = '';
					}
					$result = '<input' .
				...
				case '_stock':
					if ( 'yes' !== get_option( 'alg_wc_cog_bulk_edit_tool_manage_stock', 'no' ) ) {
						...
						$result       = '<input' .
						                ...
						                ' value="' . esc_attr( $stock_value ) . '"' . '>';
						$result       .= wc_help_tip(
						/* translators: %s: Stock status value (e.g. "instock", "outofstock", "N/A"). */
						sprintf( __( 'Stock status: %s', 'cost-of-goods-for-woocommerce' ), esc_html( $stock_status ) )
					);
					}
					break;
				default:
