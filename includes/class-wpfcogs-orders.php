...
if ( ! class_exists( 'WPFCOGS_Orders' ) ) :

class WPFCOGS_Orders {
	 ...
	 * @version 2.2.0
	 * @since   2.1.0
	 * @todo    [maybe] split into smaller files, e.g. `class-wpfcogs-orders-ajax.php`, `class-wpfcogs-orders-columns.php` etc.
	 */
	 ...
	 */
	function includes() {
		require_once( 'class-wpfcogs-orders-meta-boxes.php' );
	}
	...
	function get_options() {
		...
		$this->item_handling_fees_option     = apply_filters( 'wpfcogs_orders_item_handling_fees', 'no' );
	...
	function add_hooks() {
		...
		// Shipping to profit.
		add_filter( 'wpfcogs_update_order_values', array( $this, 'add_order_shipping_cost_to_profit' ), 10, 2 );
		add_filter( 'wpfcogs_extra_profit_meta_keys', array( $this, 'add_shipping_to_profit_meta_key_to_order_cmb' ) );

		// Fees to profit.
		add_filter( 'wpfcogs_update_order_values', array( $this, 'add_order_fees_to_profit' ), 10, 2 );
		add_filter( 'wpfcogs_extra_profit_meta_keys', array( $this, 'add_fees_to_profit_meta_key_to_order_cmb' ) );

		// Taxes to profit.
		add_filter( 'wpfcogs_update_order_values', array( $this, 'add_order_taxes_to_profit' ), 10, 2 );
		add_filter( 'wpfcogs_extra_profit_meta_keys', array( $this, 'add_taxes_to_profit_meta_key_to_order_cmb' ) );
		...
		// Payment gateways.
		add_filter( 'wpfcogs_update_order_values', array( $this, 'manage_payment_gateways' ), 10, 2 );
		add_filter( 'wpfcogs_extra_profit_meta_keys', array( $this, 'add_payment_gateways_profit_meta_key_to_order_cmb' ) );

		// Extra costs from meta.
		add_filter( 'wpfcogs_update_order_values', array( $this, 'manage_extra_costs_from_meta' ), 10, 2 );
		add_filter( 'wpfcogs_extra_profit_meta_keys', array( $this, 'add_extra_costs_from_meta_to_profit_meta_key_to_order_cmb' ) );
	...
	function handle_orders_page_columns_width() {
		$columns = array(
			array( 'key' => 'cost', 'value' => wpfcogs_get_option( 'alg_wc_cog_orders_column_cost_width', 3 ) ),
			array( 'key' => 'profit', 'value' => wpfcogs_get_option( 'alg_wc_cog_orders_column_profit_width', 3 ) ),
			array( 'key' => 'profit_percent', 'value' => wpfcogs_get_option( 'alg_wc_cog_orders_column_profit_percent', 5 ) ),
			array( 'key' => 'profit_margin', 'value' => wpfcogs_get_option( 'alg_wc_cog_orders_column_profit_margin', 5 ) ),
		);
		$style   = '';
	 ...
	 * @return mixed
	 */
	function hide_order_item_profit_meta( $meta_keys ){
		$meta_keys[] = '_alg_wc_cog_item_profit';
		return $meta_keys;
	}
	...
	function add_payment_gateways_profit_meta_key_to_order_cmb( $meta_keys ) {
		if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_order_taxes_to_profit', 'no' ) ) {
	...
	function manage_extra_costs_from_meta( $order_values, $order_info ) {
		$order_extra_cost_from_meta = wpfcogs_get_option( 'alg_wc_cog_order_extra_cost_from_meta', '' );
		if (
			...
			$consider_extra_costs_from_meta_as_positive = 'yes' === wpfcogs_get_option( 'alg_wc_cog_order_extra_cost_from_meta_as_positive', 'no' );
			foreach ( $order_extra_cost_from_meta as $meta_key ) {
				...
				$meta_fees += apply_filters( 'wpfcogs_order_extra_cost_from_meta', floatval( $fee ), $order );
			}

			if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_order_extra_cost_from_meta_as_profit', 'no' ) ) {
    ...
    function manage_payment_gateways( $order_values, $order_info ){
	    if (
		    'yes' === wpfcogs_get_option( 'alg_wc_cog_gateways_values_enabled', wpfcogs_get_gateways_option_default() ) &&
		    is_a( $order = $order_info['order'], 'WC_Order' ) &&
		    method_exists( $order, 'get_payment_method' )
	    ) {
		    $gateway_costs_fixed      = wpfcogs_get_option( 'alg_wc_cog_gateway_costs_fixed', array() );
		    $gateway_costs_percent    = wpfcogs_get_option( 'alg_wc_cog_gateway_costs_percent', array() );
		    $order_gateway            = $order->get_payment_method();
		    $gateway_cost_percent_max = wpfcogs_get_option( 'alg_wc_cog_gateway_costs_percent_max_value', array() );
		    ...
		    if ( ! empty( $gateway_costs_fixed[ $order_gateway ] ) ) {
			    $gateway_cost_fixed = apply_filters(
				    'wpfcogs_order_gateway_cost_fixed',
		    ...
		    if ( ! empty( $gateway_costs_percent[ $order_gateway ] ) ) {
			    ...
			    $gateway_cost_percent           = apply_filters(
				    'wpfcogs_order_gateway_cost_percent',
		    ...
		    // Profit.
		    $gateway_fixed_profit_arr = wpfcogs_get_option( 'alg_wc_cog_gateway_profit_fixed', array() );
	...
	function add_order_taxes_to_profit( $order_values, $order_info ) {
		if ( 'yes' === get_option( 'alg_wc_cog_order_taxes_to_profit', 'no' ) ) {
			...
			$fees                        = (float) apply_filters( 'wpfcogs_order_total_taxes', $order->get_total_tax(), $order );
	...
	function add_order_fees_to_profit( $order_values, $order_info ) {
		if ( 'yes' === get_option( 'alg_wc_cog_order_fees_to_profit', 'no' ) ) {
			$order                       = $order_info['order'];
			$fees                        = (float) apply_filters( 'wpfcogs_order_total_fees', $order->get_total_fees(), $order );
	...
	function add_order_shipping_cost_to_profit( $order_values, $order_info ) {
		if ( 'yes' === get_option( 'alg_wc_cog_order_shipping_to_profit', 'no' ) ) {
			$order                       = $order_info['order'];
			$shipping_total              = (float) apply_filters( 'wpfcogs_order_shipping_total', $order->get_shipping_total(), $order );
	...
	function shop_order_sortable_columns( $columns ) {
		if ( ! apply_filters( 'wpfcogs_create_orders_columns_validation', true ) ) {
			return $columns;
		}
	...
	function shop_order_pre_get_posts_order_by_column( $query ) {
		wpfcogs_pre_get_posts_order_by_column( $query, 'shop_order', $this->is_columns_sorting_exclude_empty_lines );
	}
	...
	function order_admin_notice() {
		if (
			...
			$admin_notice_text = get_option( 'alg_wc_cog_orders_admin_notice_text', __( 'You are selling below the cost of goods.', 'cost-of-goods-for-woocommerce' ) );
			printf( '<div class="%1$s"><p>%2$s</p></div>', esc_attr( $class ), esc_html( $admin_notice_text ) );
		}
	}
	...
	function add_order_columns( $columns ) {
		...
		if ( $this->is_columns_extra_cost_per_order && in_array( true, $this->is_order_extra_cost_per_order ) ) {
			foreach ( $this->is_order_extra_cost_per_order as $fee_type => $is_enabled ) {
				if ( $is_enabled ) {
					$this->order_columns[ $fee_type ] = ucfirst( wpfcogs()->core->extra_costs_labels->get_label( $fee_type ) );
				}
			}
		}
		...
		return wpfcogs_insert_in_array( $columns, $this->order_columns, 'order_total' );
	}
	 ...
	 * @todo    [maybe] `if ( 0 != ( $cost = wc_get_order_item_meta( $item_id, '_alg_wc_cog_item_cost' ) ) || 0 != ( $cost = wpfcogs()->core->products->get_product_cost( $product_id ) ) ) {`
	...
	function render_order_columns( $column, $order_id ) {
		...
		if ( is_array( $this->order_columns ) && in_array( $column, array_keys( $this->order_columns ) ) ) {
			...
			if ( in_array( $column, array( 'profit', 'cost' ) ) ) {
				$value = apply_filters( 'wpfcogs_order_admin_column_value', $value, array( 'invert' => true, 'order' => $order ) );
			}
			echo wp_kses_post( '' !== $value ? $this->format_order_column_value( $value, $column, apply_filters( 'wpfcogs_order_admin_column_value_format_args', array(), array( 'order' => $order ) ) ) : '' );
		}
	}
	...
	function save_cost_input_shop_order_save_post( $post_id, $post ) {
		// Verify nonce for user-facing hooks; skip for programmatic woocommerce_update_order.
		if ( 'woocommerce_update_order' !== current_filter() ) {
		...
		if ( in_array( current_filter(), get_option( 'alg_wc_cog_new_order_hooks_for_cost_update', array_keys( $this->get_new_order_hooks_for_cost_updating() ) ) ) ) {
			if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_avoid_infinite_loops', 'yes' ) ) {
	 ...
	 * @since   1.1.0
	 */
	function save_cost_input_shop_order_new( $post ) {
		if ( in_array( current_filter(), get_option( 'alg_wc_cog_new_order_hooks_for_cost_update', array_keys( $this->get_new_order_hooks_for_cost_updating() ) ) ) ) {
			$args = array(
				'is_new_order' => true,
				'order_id'     => $post,
			...
			if ( ! isset( $args['order'] ) && 2 == func_num_args() ) {
	...
	function new_order_item_ajax( $item_id, $item, $order_id ) {
		if (
			...
			( $cost = wpfcogs()->core->products->get_product_cost( $product_id ) )
		) {
	...
	function add_cost_input_shop_order( $item_id, $item, $product ) {
		if (
			...
			'WC_Order_Item_Product' === get_class( $item ) &&
			apply_filters( 'wpfcogs_add_cost_input_validation', true )
		) {
			$readonly_attr = ( 'readonly' === $this->item_costs_option ? ' readonly="readonly"' : '' );
			printf(
				'<p><label for="wpfcogs_item_cost_%1$d">%2$s (%3$s) </label><input name="wpfcogs_item_cost[%1$d]" id="wpfcogs_item_cost_%1$d" type="text" class="short wc_input_price" value="%4$s"%5$s></p>',
				absint( $item_id ),
				esc_html__( 'Cost of goods', 'cost-of-goods-for-woocommerce' ),
				esc_html( wpfcogs()->core->get_default_shop_currency_symbol() ),
	...
	function add_handling_fee_input_shop_order( $item_id, $item, $product ) {
		if (
			...
			'WC_Order_Item_Product' === get_class( $item ) &&
			apply_filters( 'wpfcogs_add_handling_fee_input_validation', true )
		) {
			$readonly_attr = ( 'readonly' === $this->item_handling_fees_option ? ' readonly="readonly"' : '' );
			printf(
				'<p><label for="wpfcogs_item_handling_fee_%1$d">%2$s (%3$s) </label><input name="wpfcogs_item_handling_fee[%1$d]" id="wpfcogs_item_handling_fee_%1$d" type="text" class="short wc_input_price" value="%4$s"%5$s></p>',
				absint( $item_id ),
	...
	function get_order_total_for_pecentage_fees( $order ) {
		switch ( $this->order_extra_cost_percent_total ) {
			case 'total_excl_tax':
				return apply_filters( 'wpfcogs_order_total_for_pecentage_fees', ( $order->get_total() - $order->get_total_tax() ), $order );
			case 'total_incl_tax':
				return apply_filters( 'wpfcogs_order_total_for_pecentage_fees', $order->get_total(), $order );
			default: // 'subtotal_excl_tax'
				return apply_filters( 'wpfcogs_order_total_for_pecentage_fees', $order->get_subtotal(), $order );
		}
	}
	...
	function get_shipping_total_for_percentage_fees( $order ) {
		...
		return apply_filters( 'wpfcogs_shipping_total_for_pecentage_fees', $total, $order );
	}
	...
	function update_order_items_costs( $args ) {
		...
		if ( ! apply_filters( 'wpfcogs_update_order_items_costs_validation', true, $order ) ) {
		...
		// Shipping classes.
		$is_shipping_classes_enabled = apply_filters( 'wpfcogs_shipping_classes_enabled', false );
		...
		$ignore_quantity = 'yes' === wpfcogs_get_option( 'alg_wc_cog_order_calculation_ignores_quantity', 'no' );
		...
		$ignore_item_refund_amount = 'yes' === get_option( 'alg_wc_cog_ignore_item_refund_amount', wpfcogs_get_ignore_item_refund_amount_default() );
		...
		$order_total_refunded = (float) apply_filters( 'wpfcogs_order_total_refunded', $order_total_refunded, $order );
		do_action( 'wpfcogs_before_update_order_items_costs', $order );
		...
		if ( empty( $this->delay_calculations_status ) || $order->has_status( $this->delay_calculations_status ) ) {
			...
			foreach ( $order->get_items() as $item_id => $item ) {
				...
				if ( $is_new_order ) {
					if ( ! $is_no_costs_only || '' === wc_get_order_item_meta( $item_id, '_alg_wc_cog_item_cost' ) ) {
						$product_id = ( ! empty( $item['variation_id'] ) ? $item['variation_id'] : $item['product_id'] );
						$cost       = apply_filters( 'wpfcogs_order_item_cost', wpfcogs()->core->products->get_product_cost( $product_id ), $product_id, $item_id, $item );
						$do_update  = ( 0 !== $cost );
					} else {
						$do_update = false;
					}
				} else {
					$cost      = ( isset( $posted['wpfcogs_item_cost'][ $item_id ] ) ? wc_clean( $posted['wpfcogs_item_cost'][ $item_id ] ) : false );
					$do_update = ( isset( $posted['wpfcogs_item_cost'][ $item_id ] ) );
				}
				if ( $do_update ) {
					wc_update_order_item_meta( $item_id, '_alg_wc_cog_item_cost', $cost );
				} else {
				...
				if ( $is_new_order ) {
					if ( '' === wc_get_order_item_meta( $item_id, '_alg_wc_cog_item_handling_fee' ) ) {
						...
						$handling_fee = apply_filters(
							'wpfcogs_order_item_handling_fee',
							wpfcogs()->core->products->get_product_handling_fee( $product_id ),
							$order,
							$product_id
						);
					...
					$handling_fee = ( isset( $posted['wpfcogs_item_handling_fee'][ $item_id ] ) ? wc_clean( $posted['wpfcogs_item_handling_fee'][ $item_id ] ) : false );
					$do_update    = ( isset( $posted['wpfcogs_item_handling_fee'][ $item_id ] ) );
				}
				...
				if ( '' !== $cost || '' !== $handling_fee ) {
					$cost = wpfcogs_sanitize_number( array(
						'value'                    => $cost,
					...
					$line_total   = apply_filters( 'wpfcogs_order_line_total', $item_line_total, $order );
					$order_profit += ( $line_total - $line_cost );
					$item_profit  += ( $line_total - $line_cost );
					$items_cost   += $line_cost;
					$total_price  += $line_total;
					// handling fee.
					$handling_fee       = wpfcogs_sanitize_number( array(
						'value'                     => $handling_fee,
						'dots_and_commas_operation' => 'comma-to-dot'
					) );
					$handling_fee       = (float) $handling_fee;
					$line_handling_fee  = $handling_fee * $quantity;
					$order_profit       -= $line_handling_fee;
					$item_profit        -= $line_handling_fee;
					...
					if (
						...
						if ( ! empty( $shipping_class_cost = $shipping_classes_fixed_opt[ $product_shipping_class_term->term_id ] ) ) {
							if (
								...
								$shipping_class_cost_fixed_total += (float) apply_filters( 'wpfcogs_order_shipping_class_cost_fixed', $shipping_class_cost, $order, $product_shipping_class_term->term_id );
								...
								$shipping_class_cost_fixed_total += (float) apply_filters( 'wpfcogs_order_shipping_class_cost_fixed', $shipping_class_cost_per_item, $order, $product_shipping_class_term->term_id );
							}
						}
						// Percent
						if ( ! empty( $shipping_class_cost = $shipping_classes_percent_opt[ $product_shipping_class_term->term_id ] ) ) {
							$shipping_class_cost_percent_total += (float) apply_filters( 'wpfcogs_order_shipping_class_cost_percent', ( $item->get_total() * $shipping_class_cost / 100 ), $order, $product_shipping_class_term->term_id );
						}
				...
				$order_item_values = apply_filters( 'wpfcogs_update_order_item_values', array(
				...
				$order_profit      = $order_item_values['order_profit'];
				$item_profit       = $order_item_values['item_profit'];
				$total_price       = $order_item_values['total_price'];
				$total_cost        = $order_item_values['total_cost'];
				$fees              = $order_item_values['fees'];
				wc_update_order_item_meta( $item_id, '_alg_wc_cog_item_profit', sprintf('%.10f', $item_profit) );
			}
			$order_values = apply_filters( 'wpfcogs_update_order_values', array(
				'profit'      => $order_profit,
				'total_price' => $total_price,
				'total_cost'  => $total_cost,
				'fees'        => $fees,
				...
				foreach ( $shipping_methods as $shipping_method ) {
					...
					if ( ! empty( $this->shipping_costs_fixed[ $shipping_method_id ] ) ) {
						$shipping_cost_fixed += apply_filters(
							'wpfcogs_order_shipping_cost_fixed',
					...
					if ( ! empty( $this->shipping_costs_percent[ $shipping_method_id ] ) ) {
						...
						$shipping_cost_percent += apply_filters(
							'wpfcogs_order_shipping_cost_percent',
			...
			if ( 0 !== $this->order_extra_cost_fixed || 0 !== $this->order_extra_cost_percent ) {
				...
				if ( 0 !== $this->order_extra_cost_fixed ) {
					$extra_cost_fixed = apply_filters(
						'wpfcogs_order_extra_cost_fixed',
			...
			if ( $order_total_refunded > 0 ) {
				if ( 'profit_based_on_total_refunded' === $refund_profit_calc_method ) {
					$order_profit -= $order_total_refunded;
					...
					$net_payment  = apply_filters( 'wpfcogs_order_net_payment', $order->get_total() - $order_total_refunded );
					$net_payment  = 'yes' === get_option( 'alg_wc_cog_net_payment_inclusive_of_tax', 'no' ) ? $net_payment : $net_payment * $tax_percent;
					$order_profit = $net_payment - $total_cost;
				}
			}
		}
		// Final filters.
		$total_cost = apply_filters( 'wpfcogs_order_cost', $total_cost, $order );
		$order_profit = apply_filters( 'wpfcogs_order_profit', $order_profit, $order );
		// Order items
		...
		$order->update_meta_data( '_alg_wc_cog_order_profit_margin', ( 0 != $total_price ? ( $order_profit / $total_price * 100 ) : 0 ) );
		...
		$order->save();
		do_action( 'wpfcogs_update_order_values_action', array(
			'order_id' => $order_id,
			'order'    => $order,
		) );
