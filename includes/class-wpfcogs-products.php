...
class WPFCOGS_Products {
	...
	function add_hooks() {
		...
		// Shortcodes.
		add_shortcode( 'wpfcogs_product_profit', array( $this, 'sc_wpfcogs_product_profit' ) );
		add_shortcode( 'wpfcogs_product_cost', array( $this, 'sc_wpfcogs_product_cost' ) );
	}

	/**
	 * wpfcogs_product_cost.
	 *
	 * @version 4.1.1
	 * @since   3.1.0
	 *
	 ...
	 * @return string
	 */
	function sc_wpfcogs_product_cost( $atts ) {
		if ( 'no' === get_option( 'alg_wc_cog_shortcode_product_cost', 'no' ) ) {
			return '[wpfcogs_product_cost]';
		}
		$atts          = shortcode_atts( array(
			'product_id'    => get_the_ID(),
			'html_template' => '<span class="wpfcogs-product-cost">{content}</span>',
		), $atts, 'wpfcogs_product_cost' );
	 ...
	 * @return string
	 */
	function sc_wpfcogs_product_profit( $atts ) {
		if ( 'no' === get_option( 'alg_wc_cog_shortcode_product_profit', 'no' ) ) {
			return '[wpfcogs_product_profit]';
		}
		$atts = shortcode_atts( array(
			'product_id'      => get_the_ID(),
			...
			'html_template'   => '<span class="wpfcogs-product-profit">{content}</span>',
		), $atts, 'wpfcogs_product_profit' );
	...
	function sanitize_cog_meta( $value ) {
		...
		return wpfcogs_sanitize_cost( array(
			'value' => $value,
		) );
	}
		...
		if ( isset( $data['meta_data'] ) && is_array( $data['meta_data'] ) && ! empty( $data['meta_data'] ) ) {
			foreach ( $data['meta_data'] as $key => $value ) {
				...
				if ( '_alg_wc_cog_cost' === $value['key'] ) {
					...
					if ( 'yes' === wpfcogs_get_option( 'alg_wc_cog_import_csv_normalize_cost', 'no' ) ) {
						$final_value = wpfcogs_normalize_price( $final_value );
					}
	...
	function product_sortable_columns( $columns ) {
		if ( ! apply_filters( 'wpfcogs_create_product_columns_validation', true ) ) {
			return $columns;
		}
	...
	function product_pre_get_posts_order_by_column( $query ) {
		wpfcogs_pre_get_posts_order_by_column( $query, 'product', $this->is_columns_sorting_exclude_empty_lines );
	}
	...
	function add_product_columns( $columns ) {
		...
		return wpfcogs_insert_in_array( $columns, $this->product_columns, 'price' );
	}
	...
	function get_product_cost( $product_id, $args = null ) {
		...
		return apply_filters( 'wpfcogs_get_product_cost', $cost, $product_id, isset( $parent_id ) ? $parent_id : null, $args );
	}
	...
	function get_product_handling_fee( $product_id, $args = null ) {
		...
		return apply_filters( 'wpfcogs_get_product_handling_fee', (float) str_replace( ',', '.', $handling_fee ), $product_id, isset( $parent_id ) ? $parent_id : null, $args );
	}
	...
	function get_product_cost_html( $product_id ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			return wp_kses_post( '' === ( $cost = $this->get_product_cost( $product_id ) ) ? '' : wpfcogs_format_cost( $cost ) );
		}
	}
	...
	function update_product_price_by_profit( $args = array() ) {
		...
		$product         = wc_get_product( $product_id );
		$product_cost    = wpfcogs()->core->products->get_product_cost( $product->get_id() );
		$new_price       = 0;
