...
if ( ! class_exists( 'WPFCOGS' ) ) :
	...
	final class WPFCOGS {
		...
		/**
		 * @since 1.0.0
		 * @var   WPFCOGS The single instance of the class
		 */
		...
		/**
		 * Main WPFCOGS Instance
		 *
		 * Ensures only one instance of WPFCOGS is loaded or can be loaded.
		 *
		 * @version 1.0.0
		 * @since   1.0.0
		 * @static
		 * @return  WPFCOGS - Main instance
		 */
		...
		function init() {
			...
			if ( 'cost-of-goods-for-woocommerce-pro.php' === basename( $this->get_filesystem_path() ) ) {
				$this->pro = require_once( 'pro/class-wpfcogs-pro.php' );
			}
		...
		function add_cross_selling_library() {
			...
			require_once plugin_dir_path( wpfcogs()->get_filesystem_path() ) . 'vendor/autoload.php';
		...
		function handle_documentation_params( $params ) {
			...
			$params['pro_settings_filter'] = 'wpfcogs_settings';
		...
		function includes() {
			// Functions.
			require_once( 'wpfcogs-functions.php' );

			// Load deprecated functions, hooks, and shortcodes.
			require_once( 'wpfcogs-deprecated-functions.php' );
			require_once( 'wpfcogs-deprecated-hooks.php' );
			require_once( 'wpfcogs-deprecated-shortcodes.php' );

			// Core
			$this->core = require_once( 'class-wpfcogs-core.php' );
		}
		...
		function action_links( $links ) {
			$custom_links   = array();
			$custom_links[] = '<a href="' . admin_url( 'admin.php?page=wc-settings&tab=alg_wc_cost_of_goods' ) . '">' . esc_html__( 'Settings', 'woocommerce' ) . '</a>';
			...
			$custom_links[] = '<a href="' . admin_url( 'tools.php?page=bulk-edit-costs' ) . '">' . esc_html__( 'Bulk edit costs', 'woocommerce' ) . '</a>';
			$custom_links[] = '<a href="' . admin_url( 'tools.php?page=bulk-edit-prices' ) . '">' . esc_html__( 'Bulk edit prices', 'woocommerce' ) . '</a>';
		...
		function add_woocommerce_settings_tab( $settings ) {
			if ( ! apply_filters( 'wpfcogs_create_wc_settings_tab_validation', true ) ) {
				return $settings;
			}
			$settings[] = require_once( 'settings/class-wpfcogs-settings.php' );
		...
		function version_updated() {
			do_action( 'wpfcogs_on_update', $this->version, get_option( 'alg_wc_cog_version', '' ) );
