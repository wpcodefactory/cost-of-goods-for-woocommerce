...
if ( ! class_exists( 'WPFCOGS_Orders_Meta_Boxes' ) ) :

class WPFCOGS_Orders_Meta_Boxes {
	...
	function __construct() {
		...
		// Manual order cost.
		add_filter( 'wpfcogs_order_cost', array( $this, 'save_order_cost_manually' ) );
		add_filter( 'wpfcogs_order_profit', array( $this, 'calculate_profit_from_manual_cost' ) );
		add_filter( 'wpfcogs_order_metabox_cost_value_html', array( $this, 'replace_order_metabox_cost_by_input' ), 10, 2 );
	}
	...
	function replace_order_metabox_cost_by_input( $cost_value_html, $cost ) {
		if ( 'yes' === get_option( 'alg_wc_cog_edit_order_cost_manually', 'no' ) ) {
			$cost_value_html = '<input style="color:red;" type="number" step="0.000001" name="wpfcogs_order_cost_input" id="" value="' . esc_attr( $cost ) . '"/>';
		}
	...
	function save_order_cost_manually( $order_cost ) {
		if ( isset( $_POST['wpfcogs_order_cost_input'] ) ) {
		...
		if (
			'yes' === get_option( 'alg_wc_cog_edit_order_cost_manually', 'no' ) &&
			isset( $_POST['wpfcogs_order_cost_input'] ) &&
			'' !== ( $order_cost_input = sanitize_text_field( wp_unslash( $_POST['wpfcogs_order_cost_input'] ) ) ) &&
	...
	function add_order_meta_box() {
		if ( ! apply_filters( 'wpfcogs_create_order_meta_box_validation', true ) ) {
			return;
		}
		$screen = wpfcogs()->core->orders->get_shop_order_screen_id();
		if ( wpfcogs()->core->orders->is_order_meta_box ) {
			add_meta_box( 'wpfcogs',
	...
	function render_order_meta_box( $post ) {
		...
		$cost                = apply_filters( 'wpfcogs_order_metabox_value', $cost, array( 'invert' => true, 'order' => $order ) );
		$handling_fee        = $order->get_meta( '_alg_wc_cog_order_' . 'handling_fee', true );
		$profit              = $order->get_meta( '_alg_wc_cog_order_' . 'profit', true );
		$profit              = apply_filters( 'wpfcogs_order_metabox_value', $profit, array( 'invert' => true, 'order' => $order ) );
		...
		$profit_placeholders = array(
			'%profit%'         => wpfcogs()->core->orders->format_order_column_value( $profit, 'profit', apply_filters( 'wpfcogs_order_metabox_value_format_args', array(), array( 'order' => $order ) ) ),
			'%profit_percent%' => wpfcogs()->core->orders->format_order_column_value( $profit_percent, 'profit_percent' ),
			'%profit_margin%'  => wpfcogs()->core->orders->format_order_column_value( $profit_margin, 'profit_margin' ),
		);
		...
		$cost_html           = apply_filters( 'wpfcogs_order_metabox_cost_value_html', wpfcogs_format_cost( $cost, apply_filters( 'wpfcogs_order_metabox_value_format_args', array(), array( 'order' => $order ) ) ), $cost, $order_id );
		$table_data          = array(
		...
		echo wp_kses( wpfcogs_get_table_html( $table_data, $table_args ), $allowed_html );
		// Cost details
		$table_data     = array();
		...
		$cost_meta_keys = apply_filters( 'wpfcogs_cost_meta_keys', $cost_meta_keys );
		foreach ( $cost_meta_keys as $key => $value ) {
			$cost = $order->get_meta( $key, true );
			$cost = apply_filters( 'wpfcogs_order_metabox_value', $cost, array( 'invert' => true, 'order' => $order ) );
			if ( ! empty( $cost ) && 0 != $cost ) {
				$table_data[] = array( $value, wpfcogs_format_cost( $cost, apply_filters( 'wpfcogs_order_metabox_value_format_args', array(), array( 'order' => $order ) ) ) );
			}
		}
		...
		$extra_profit_meta_keys = apply_filters( 'wpfcogs_extra_profit_meta_keys', $extra_profit_meta_keys );
	...
	function add_order_extra_cost_meta_box() {
		if ( ! apply_filters( 'wpfcogs_create_order_meta_box_validation', true, 'extra_cost' ) ) {
			return;
		}
		if ( in_array( true, wpfcogs()->core->orders->is_order_extra_cost_per_order ) ) {
			$screen = wpfcogs()->core->orders->get_shop_order_screen_id();
			add_meta_box( 'wpfcogs-extra-cost',
...
return new WPFCOGS_Orders_Meta_Boxes();
